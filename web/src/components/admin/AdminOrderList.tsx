...
import { updateOrderStatus, createAdminOrder, updateAdminOrder } from "@/actions/orders";
import { useState, useMemo } from "react";
import { ChevronDown, ChevronUp, Package, Plus, X, Trash2, Edit2, Loader2 } from "lucide-react";
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
            ...
            {orders.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    {orders.map(order => (
                        <OrderRow key={order.id} order={order} onStatusChange={handleStatusChange} onEdit={openEdit} />
                    ))}
                </div>
            )}
...
function OrderRow({ order, onStatusChange, onEdit }: { order: Order, onStatusChange: (id: string, s: string) => void, onEdit: (o: Order) => void }) {
